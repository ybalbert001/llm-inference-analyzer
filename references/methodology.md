...
> 目录：一、技术思路（显存拆解：config 公式 + safetensors 精确值 / 混合精度与 sub-byte 打包识别 / KV cache / activation）· 一B、并行切分口径（TP/PP/EP）· 一C、性能 Roofline · 二、验证结果 · 三、注意事项与已知局限 · 四、关键细节备忘

输入一个 HuggingFace 模型 ID，自动拆解该模型部署所需的 GPU 显存，输出终端报告和一个三 TAB 的可交互单文件 HTML：

- **TAB 1「显存拆解」（design_1）**：静态权重（逐部件）+ 运行时内存（KV cache、activation）
- **TAB 2「并行切分」（design_2）**：权重 + KV 按 TP/PP/EP 切分到 AWS GPU 节点，逐卡显示占用与剩余
- **TAB 3「性能 Roofline」（design_3）**：Decode/Prefill 的 kernel 算术强度、理论瓶颈与性能上界
...
| `*.html`（生成物） | 单文件、无外部依赖、离线可交互、自动适配深色模式；`#parallel` hash 直达并行 TAB |

三个 TAB 共用一份嵌入页面的 `viz_json` 数据；顶部筛选行的 context / 并发 / KV 精度为全局共享。`template.js` 在生成时内联，因此输出仍是可离线打开的单文件。
...
- `html_output/check_parallel.py`：机械自检脚本，从生成 HTML 提取 viz_json，用 Python 复算并断言守恒律——Σ(各卡权重) = 模型总量 + 闭式复制冗余；KV 复制倍数 = TP（MLA）或 TP/min(TP, n_kv)（GQA）。GLM-5.2 与 Qwen3-32B 全部通过（dev < 1e-15）。

---

## 一C、性能 Roofline（Roofline TAB，design_3）

回答"跑起来受带宽限制还是算力限制、上界是多少、优化投给谁"。图上只画 **Decode（当前并发）** 和 **Prefill（8K chunk）** 两个聚合点（落在屋顶线上，强度 = 各部件 FLOPs 之和 ÷ 字节之和）；每个点下方一张卡片，内含**部件拆解表**（Dense GEMM / MoE experts / KV 读取 × TFLOPs / HBM 读写 / 强度 / 受限于 / 时间占比）+ 吞吐上界推导 + 处方文案。`#roofline` hash 直达。
...
Prefill 的 KV HBM 字节按 KV 一次读取/写入计算，是不包含 FlashAttention tiling 和 DSA gather 重读的理想下界。部件时间 = max(字节/带宽, FLOPs/峰值)，**时间占比列直接指出优化对象**。
...
判定：聚合强度 < 拐点（峰值÷带宽）→ memory-bound。峰值按权重主导 dtype 选（fp8 → fp8 峰值；GPU 不支持时回退 bf16 并注明）。强度与 TP 无关，绝对时间随 TP 缩短。
