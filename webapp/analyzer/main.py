...
def build_raw_evidence(a: dict, cfg: dict) -> str:
    """Section 0 of the evidence tab: the primary source files, verbatim, beside
    the facts the tool parsed from them. Left = raw config.json (key fields +
    collapsible full text), the index.json bridge file, and a representative
    safetensors tensor sample; right = parsed architecture facts + dtype mix + Σ
    cross-check. A draggable gutter sits between the two panes. Returns '' if
    safetensors was not read (the pane's whole point is the file-vs-tool
    comparison)."""
...
def _build_lang_fragments(a: dict, cfg: dict, p: dict, is_moe: bool, short: str,
    ...
    return {
        ...
        "lbl_cp": t("html.lbl_cp"),
        "cp_title": t("html.cp_title"),
